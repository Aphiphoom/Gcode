! function() {
        ...
        i.innerHTML = `\n        <td>${n.email}</td>\n        <td><span class="status-pill status-${n.status}">${r=n.status,{pending:"รออนุมัติ",active:"ใช้งานได้",suspended:"ระงับสิทธิ์"}[r]||r}</span></td>\n        <td>${n.expiresAt?n.expiresAt.slice(0,10)+(l?" (หมดอายุ)":""):"—"}</td>\n        <td>${n.role}</td>\n        <td>${n.createdAt?n.createdAt.slice(0,10):"—"}</td>\n        <td><button class="mini" data-id="${n.id}">จัดการ</button></td>`, i.querySelector("button").addEventListener("click", () => function(n) {
          a = n.id, t("detailPanel").style.display = "", t("detailEmail").textContent = n.email, t("detailStatus").value = n.status, t("detailExpires").value = n.expiresAt ? n.expiresAt.slice(0, 10) : "", t("detailRole").value = n.role, t("saveUserMsg").textContent = "", async function(n) {
            ...
            const {
              data: a,
              error: s
            } = await e.from("login_logs").select("*").eq("user_id", n).order("created_at", {
              ascending: !1
            }).limit(50);
            if (s) return void(t("logTableBody").innerHTML = `<tr><td colspan="5" class="empty-hint">โหลด log ไม่สำเร็จ: ${s.message}</td></tr>`);
            ! function(e) {
              const n = t("logTableBody");
              if (!e.length) return void(n.innerHTML = '<tr><td colspan="5" class="empty-hint">ยังไม่มีประวัติการ login</td></tr>');
              n.innerHTML = "", e.forEach(t => {
  ...
  async function i() {
    if (!a) return;
    t("saveUserMsg").textContent = "กำลังบันทึก...";
    const n = t("detailExpires").value ? new Date(t("detailExpires").value + "T23:59:59Z").toISOString() : null,
      {
        error: i
      } = await e.from("profiles").update({
        status: t("detailStatus").value,
        role: t("detailRole").value,
        expires_at: n
      }).eq("id", a);
    i ? t("saveUserMsg").textContent = "⚠ บันทึกไม่สำเร็จ: " + i.message : (t("saveUserMsg").textContent = "✓ บันทึกแล้ว (มีผลตอนสมาชิก login ครั้งถัดไป)", await s())
  }!async function() {
    e = window.AuthClient.sb;
    const n = await window.AuthClient.requireLogin();
    if (!n) return;
