  ...
  function defaultMachine() {
    return {
      ...
      smallPartFinalPass: 2,   // ความหนาของรอบตัดก่อนสุดท้ายสำหรับชิ้นเล็ก (mm)
                               ...
                               // 0 = ไม่เพิ่มรอบพิเศษ
      smallPartFinalFeed: 0    // ความเร็วตัด (mm/min) สำหรับ pass สุดท้ายของชิ้นเล็กโดยเฉพาะ
                               // 0 = ใช้ feedXY ปกติของดอกกัดตัวนั้น (ไม่ override)
    };
  }
...
M30`;
  ...
  function guessMapping(layerName, tools, machine) {
    ...
    // mark_square: ตรวจก่อนเงื่อนไขอื่น เพราะชื่อขึ้นต้น "mark_square" ชัดเจน
    if (up.startsWith('MARK_SQUARE')) operation = 'Mark Square';
    else if (up.includes('POCKET')) operation = 'Pocket';
    ...
    let toolNumber;
    let depth;
    if (operation === 'Mark Square') {
      toolNumber = findOutsideTool(tools);
      depth = 'pt+cd'; // ตัดทะลุเหมือน Profile Outside (สำหรับ Bottom file)
    } else if (operation.indexOf('Profile') === 0) {
