...
(function (global) {
  ...
  function generate(job, config) {
    ...
    function emitContour(op) {
      ...
      // ถ้าเป็นชิ้นเล็ก (isSmallPart) และตั้ง smallPartFinalFeed ไว้ → override feed ที่ pass สุดท้าย
      ...
      op.passes.forEach((zCut, idx) => {
        ...
        if (op.tabs && op.tabs.length) {
          ...
          if (idx > 0) feed(startPt.x, startPt.y, undefined, feedXY);
          ...
          if (idx > 0) emitReverseRetrace(op.path, fromZ, feedXY);
          if (isLast) emitOpenLastPass(op.path, fromZ, zCut, feedXY);
          else emitRampedPathWithArcs(op.path, op.arcRanges, fromZ, zCut, feedXY);
        } else {
          if (idx > 0) feed(startPt.x, startPt.y, undefined, feedXY); // กลับจุดเริ่มที่ความสูงเดิม (ไม่ยก)
          emitRampedPathWithArcs(op.path, op.arcRanges, fromZ, zCut, feedXY);
          ...
          if (isLast) emitRampCloseOff(op.path, fromZ, zCut, feedXY);
        }
        fromZ = zCut;
      });
      rapid(undefined, undefined, safeZ);
    ...
    function emitMark(op) {
      ...
      op.passes.forEach((zCut, idx) => {
        ...
        if (isLast) {
          // pass สุดท้าย: ลงแบบ ramp ครึ่งเส้นแรก แล้ว ย้อนกลับจุดเริ่มที่ระดับเต็ม แล้วเดินเต็มเส้น
          emitRampedPathWithArcs(op.path, null, fromZ, zCut, finalFeedVal);
          for (let i = op.path.length - 2; i >= 0; i--) feed(op.path[i].x, op.path[i].y, zCut, finalFeedVal);
          for (let i = 1; i < op.path.length; i++) feed(op.path[i].x, op.path[i].y, zCut, finalFeedVal);
        } else {
          emitRampedPathWithArcs(op.path, null, fromZ, zCut, t.feedXY);
        }
        fromZ = zCut;
      });
      rapid(undefined, undefined, safeZ);
